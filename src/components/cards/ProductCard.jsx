    ...
    >
      ...
      <div className="relative aspect-[4/3] max-h-[110px] sm:max-h-[150px] w-full overflow-hidden bg-slate-100 dark:bg-slate-900/60 border-b border-slate-100 dark:border-slate-700/60">
        ...
        {badge && (
          <div className="absolute top-1.5 left-1.5 z-10">
            <span className="bg-secondary text-slate-950 px-1 py-0.5 text-[8px] font-black rounded shadow-2xs uppercase tracking-wider">
              {badge}
            </span>
          </div>
        )}
        ...
        <button
          ...
          className="absolute top-1.5 right-1.5 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xs p-1 rounded-full text-text-muted dark:text-slate-300 hover:text-red-500 transition-colors shadow-2xs z-20 focus:outline-none cursor-pointer"
        >
      ...
      <div className="p-1.5 sm:p-2 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-1 mb-0.5">
          <h3
            className="font-semibold text-slate-800 dark:text-slate-100 group-hover:text-primary dark:group-hover:text-blue-400 transition-colors line-clamp-1 text-[11px] sm:text-xs flex-1 leading-snug"
            title={title}
          >
            {title}
          </h3>
        ...
        <div className="flex items-baseline gap-1 mt-0.5">
          ...
          {originalPrice && (
            <span className="text-[8px] sm:text-[10px] text-text-light dark:text-slate-400 line-through font-medium">
              ₹{originalPrice.toString().replace("₹", "")}
            </span>
          )}
          {discount && (
            <span className="text-[7px] sm:text-[8px] font-extrabold text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950/60 px-0.5 rounded ml-auto">
              {discount}
            </span>
          )}
        </div>
        ...
        <div className="mt-1 pt-0.5 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between text-[8px] sm:text-[9px] text-text-muted dark:text-slate-400 font-medium">
          <div className="flex items-center gap-0.5 truncate max-w-[65%]">
            <MapPin size={8} className="shrink-0 text-slate-400" />
            <span className="truncate">{location.split(",")[0]}</span>
          </div>
          {postedTime && (
            <span className="shrink-0 text-[8px] sm:text-[9px] text-slate-400">{postedTime}</span>
          )}
        </div>
      </div>
    </Link>
